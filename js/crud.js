  ...
  tasks.forEach((e) => {
    list_container.insertAdjacentHTML(
      ...
      `
        <div class="item">
            <div class="right">
              <input class="check ilin" id="check-${e.id}" type="checkbox" ${e.complete ? "checked" : ""}>
              <p>${e.title}</p>
            </div>
            <div class="left">
              <img id="detail-${e.id}" class="detail-btn" src="img/eye.svg" alt="" />
              <img id="edit-${e.id}" class="edit-btn" src="img/edit.svg" alt="" />
              <img id="delete-${e.id}" class="delete-btn" src="img/delete.svg" alt="" />
            </div>
          </div>
        `
    );
  ...
  if (res) {
    ...
    task_detail_container.insertAdjacentHTML(
      ...
      `
        ...
        <div class="task-detail__complete">completo : ${res.complete ? "SI" : "NO"}</div>
    `
    );
  }
};
